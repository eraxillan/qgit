...
Git::TreeEntry::TreeEntry(SCRef n, SCRef s, SCRef t) : name(n), sha(s), type(t) {
...
void Git::setDefaultModel(FileHistory *fh) {
...
void Git::formatPatchFileHeader(QString* rowName, SCRef sha, SCRef diffToSha, bool combined, bool allMergeFiles) {
...
Annotate* Git::startAnnotate(FileHistory* fh, QObject* guiObj) { // non blocking
    ...
    if (!ann->start(fh)) // non blocking call
        return nullptr; // ann will delete itself when done
...
const FileAnnotation* Git::lookupAnnotation(Annotate* ann, SCRef sha) {
    ...
    return (ann ? ann->lookupAnnotation(sha) : nullptr);
}
...
const Revision* Git::revLookup(const ShaString& sha, const FileHistory* fh) const {
    ...
    const RevisionMap& r = (fh ? fh->revs : revData->revs);
    return (sha.latin1() ? r.value(sha) : nullptr);
}

bool Git::run(SCRef runCmd, QString* runOutput, QObject* receiver, SCRef buf) {

    QByteArray ba;
    bool ret = run(runOutput ? &ba : nullptr, runCmd, receiver, buf);
    if (runOutput) {
        *runOutput = ba;
    }
...
bool Git::isCommittingMerge() const {

    return isMergeHead;
}

bool Git::isStGITStack() const { return isStGIT; }
...
const QString Git::filePath(const RevFile &rf, uint i) const {

    return dirNamesVec[rf.dirAt(i)] + fileNamesVec[rf.nameAt(i)];
}

void Git::setCurContext(Domain *d) {

    curDomain = d;
}

Domain *Git::curContext() const {
...
MyProcess* Git::getFile(SCRef fileSha, QObject* receiver, QByteArray* result, SCRef fileName) {
    ...
    */
    ...
    if (!receiver) {
        run(result, runCmd);
        return nullptr; // in case of sync call we ignore run() return value
    }
    return runAsync(runCmd, receiver);
}
...
bool Git::saveFile(SCRef fileSha, SCRef fileName, SCRef path) {
    ...
    QByteArray fileData;
    getFile(fileSha, nullptr, &fileData, fileName); // sync call
...
bool Git::isUnknownFiles() const {

    return (workingDirInfo.otherFiles.count() > 0);
}

bool Git::isTextHighlighter() const {

    return isTextHighlighterFound;
}

const QString Git::textHighlighterVersion() const {

    return textHighlighterVersionFound;
}

bool Git::isMainHistory(const FileHistory *fh) {
...
const QString Git::getDesc(SCRef sha, QRegExp& shortLogRE, QRegExp& longLogRE, bool showHeader, FileHistory* fh) {
...
bool Git::getPatchFilter(SCRef exp, bool isRegExp, ShaSet& shaSet) {
    ...
    if (!run(runCmd, &runOutput, nullptr, buf))
        return false;
...
const QString Git::getCurrentBranchName() const {
...
bool Git::init(SCRef wd, bool askForRange, const QStringList* passedArgs, bool overwriteArgs, bool* quit) {
    ...
    *quit = false;
    ...
    /* we only update filtering info here, original arguments
         ...
         */
    loadArguments.filteredLoading = (!overwriteArgs && passedArgs != nullptr);
    if (loadArguments.filteredLoading)
        loadArguments.filterList = *passedArgs;

    if (overwriteArgs) // in this case must be passedArgs != nullptr
...
Git::Reference::Reference() : type(0) {

}

void Git::WorkingDirInfo::clear() {

    diffIndex = diffIndexCached = "";
    otherFiles.clear();
}

Git::FileNamesLoader::FileNamesLoader() : rf(nullptr) {
}
