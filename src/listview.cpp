...
void getTagMarkParams(QString &name, QStyleOptionViewItem& o, const int type, const bool isCurrent);
uint refTypeFromName(SCRef name);

using namespace QGit;

ListView::ListView(QWidget* parent) : QTreeView(parent), d(nullptr), git(nullptr), fh(nullptr), lp(nullptr), dropInfo(nullptr) {
...
QString ListView::refNameAt(const QPoint &pos) {
    ...
    for (RefNameIterator it(sha(index.row()), git); it.valid(); it.next()) {
        ...
        if (pos.x() <= ofs) {
            ...
            switch (it.type()) {
            case Git::BRANCH: return it.name();
            case Git::TAG: return "tags/" + it.name();
            case Git::RMT_BRANCH: return "remotes/" + it.name();
            case Git::REF: return "bases/" + it.name();
