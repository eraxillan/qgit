...
MainImpl::MainImpl(SCRef cd, QWidget* p) : QMainWindow(p) {
    ...
    lineEditSHA = new QLineEdit(nullptr);
    lineEditFilter = new QLineEdit(nullptr);
    cmbSearch = new QComboBox(nullptr);
...
void MainImpl::setRepository(SCRef newDir, bool refresh, bool keepSelection,
    ...
    */
    ...
    if (ActFilterTree->isChecked() && passedArgs == nullptr)
...
const QString MainImpl::getRevisionDesc(SCRef sha) {
    ...
    return git->getDesc(sha, shortLogRE, longLogRE, showHeader, nullptr);
}

const QString MainImpl::currentDir() const {
...
using RefGroupMap = QMap<QString, QStringList>;
...
void ExternalDiffProc::on_finished(int, QProcess::ExitStatus) { deleteLater(); }
...
ExternalEditorProc::ExternalEditorProc(QObject *p)
    : QProcess(p) {
...
ExternalEditorProc::~ExternalEditorProc() {

    terminate();
}

void ExternalEditorProc::on_finished(int, QProcess::ExitStatus) {
