...
const QString StateInfo::sha(bool n) const {

    return (n ? curS.sha : prevS.sha);
}

const QString StateInfo::fileName(bool n) const {

    return (n ? curS.fn : prevS.fn);
}

const QString StateInfo::diffToSha(bool n) const {

    return(n ? curS.dtSha : prevS.dtSha);
}

bool StateInfo::selectItem(bool n) const {

    return (n ? curS.sel : prevS.sel);
}

bool StateInfo::isMerge(bool n) const {

    return (n ? curS.isM : prevS.isM);
}

bool StateInfo::allMergeFiles(bool n) const {

    return (n ? curS.allM : prevS.allM);
}

void StateInfo::setSha(const QString &s) {

    if (isLocked) nextS.sha = s; else curS.sha = s;
}

void StateInfo::setFileName(const QString &s) {

    if (isLocked) nextS.fn = s; else curS.fn = s;
}

void StateInfo::setDiffToSha(const QString &s) {
...
void StateInfo::setSelectItem(bool b) {

    if (isLocked) nextS.sel = b; else curS.sel = b;
}

void StateInfo::setIsMerge(bool b) {
...
void StateInfo::setAllMergeFiles(bool b) {
...
bool StateInfo::requestPending() const {
...
Domain::Domain(MainImpl* m, Git* g, bool isMain) : QObject(m), git(g) {
    ...
    busy = linked = false;
    popupType = 0;
    container = new QWidget(nullptr); // will be reparented to m()->tabWdg
}
...
UpdateDomainEvent::~UpdateDomainEvent() {
