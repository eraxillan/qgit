...
*/
#ifndef QGIT_GIT_H_INCLUDED
#define QGIT_GIT_H_INCLUDED
...
public:
    ...
    struct TreeEntry {
        TreeEntry(SCRef n, SCRef s, SCRef t);
        bool operator<(const TreeEntry&) const;
        QString name;
        QString sha;
        QString type;
    };
    using TreeInfo = QList<TreeEntry>;

    void setDefaultModel(FileHistory* fh);
    ...
    const QString textHighlighterVersion() const;
    ...
    const QString getCurrentBranchName() const;
    ...
    const Revision* revLookup(const ShaString& sha, const FileHistory* fh = nullptr) const;
    const Revision* revLookup(SCRef sha, const FileHistory* fh = nullptr) const;
    ...
    bool merge(SCRef into, SCList sources, QString* error = nullptr);
...
private:
    ...
    using RefMap = QHash<ShaString, Reference>;
    ...
    void init2();
    bool run(SCRef cmd, QString* out = nullptr, QObject* rcv = nullptr, SCRef buf = "");
    bool run(QByteArray* runOutput, SCRef cmd, QObject* rcv = nullptr, SCRef buf = "");
    MyProcess* runAsync(SCRef cmd, QObject* rcv, SCRef buf = "");
    MyProcess* runAsScript(SCRef cmd, QObject* rcv = nullptr, SCRef buf = "");
